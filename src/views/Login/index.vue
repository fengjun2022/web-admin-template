  ...
  <div class="bg-login">
    <div class="bg-login-card flex">
      ...
      <div class="w-[45%] h-full relative rounded-r-[16px] card-login-bg">
        <div class="text-center w-full mt-14 text-xl font-bold">
          后台管理系统
        </div>
        <div class="login-form">
          ...
          >
            ...
            <div class="mt-[-5px]">
              <el-checkbox
                  @click="isSave"
                  v-model="loginSettingStore.pswChecked"
                  label="记住密码"
                  size="large"
              />
            </div>
...
import type {loginForm, userTokenAndPermission} from "@/types/login";
...
const onSubmit = async () => {
  ...
  try {
    ...
    userInfoStore.nameChange(res.data.userName)
    loginSettingStore.changeSlideSyncFlag();
    loginSettingStore.submitNumReset();
    const toPath = (router.query.redirect as string) || "/";
    await routes.push({path: toPath});
  } catch (e) {
    loginSettingStore.submitNumChange()
    loginSettingStore.changeSlideSyncFlag(false)
  }
...
.bg-login {
  background: url("@/assets/images/login/bg-login.jpg") no-repeat;
